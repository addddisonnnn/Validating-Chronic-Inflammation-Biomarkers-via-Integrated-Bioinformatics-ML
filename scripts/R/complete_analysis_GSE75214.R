# Save as: complete_analysis_GSE75214.R
...
BiocManager::install(c("oligo", "limma", "hugene10sttranscriptcluster.db", "ggplot2", "dplyr"))

library(oligo)
library(limma)
library(hugene10sttranscriptcluster.db)  # Correct annotation package for GPL6244
library(ggplot2)
library(dplyr)

# Set paths
cel_path <- "/projectnb/bf528/students/addisony/project1/Validating-Chronic-Inflammation-Biomarkers-via-Integrated-Bioinformatics-ML-Paper/data/raw/GSE75214"
output_dir <- "/projectnb/bf528/students/addisony/project1/Validating-Chronic-Inflammation-Biomarkers-via-Integrated-Bioinformatics-ML-Paper/results/GSE75214"
...
group_info <- read.csv("GSE75214_sample_groups_fixed.csv")
analysis_samples <- group_info %>%
  filter(!is.na(Group)) %>%
  filter(Group %in% c("CD", "Control"))
...
# Extract full filenames without extension
...
cat("\nFirst few matched files:\n")
print(head(names(selected_cel_files)))
...
# For hugene10sttranscriptcluster.db, use "SYMBOL" as column
...
output_file <- file.path(output_dir, "GSE75214_normalized_gene_expression.csv")
...
contrast_matrix <- makeContrasts(CD - Control, levels = design)
...
cat("Upregulated in CD:", sum(deg_sig$logFC > 0), "\n")
cat("Downregulated in CD:", sum(deg_sig$logFC < 0), "\n")

# 10. Save DEG results
deg_file <- file.path(output_dir, "GSE75214_DEGs.csv")
...
summary_file <- file.path(output_dir, "GSE75214_analysis_summary.csv")
